...
from compute_horde_validator.validator.models import Miner, SyntheticJobBatch, SyntheticJob
from compute_horde_validator.validator.scoring import score_batches, ExecutorClass
...
def test_score_batches_with_changed_params_avg(setup_data, settings):
    ...
    original_settings = {
        'HORDE_SCORE_AVG_PARAM': settings.HORDE_SCORE_AVG_PARAM,
        'HORDE_SCORE_SIZE_PARAM': settings.HORDE_SCORE_SIZE_PARAM,
        'HORDE_SCORE_CENTRAL_SIZE_PARAM': settings.HORDE_SCORE_CENTRAL_SIZE_PARAM,
    }

    settings.HORDE_SCORE_AVG_PARAM = 2  # Increase importance of average score (use avg**2 for better effect)
